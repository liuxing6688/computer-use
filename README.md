...
相邻两次输入动作（点击、文本输入）至少隔 0.5 秒，服务端在注入前等待，不靠模型自己放慢。只读工具不占这个间隔，也不把连续计数清零。

急停热键是 **Ctrl+Break**。按下后，正在等待间隔、尚未注入的输入全部取消；之后所有输入工具直接拒绝，直到调用 `resume` 并在 Claude Code 里确认。只读工具和声明任务作用域在急停后仍可用。已交给系统注入的那一下收不回来。
