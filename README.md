...
| `observe_window` | 截取一个窗口（不截全屏）。返回并列的目标清单 `targets`（像素通道下为空）与采集元数据 `metadata`（截图 ID、采集时刻、缩放比、DPI 缩放、屏幕偏移） |
| `zoom` | 把某张截图上的矩形按屏幕原尺寸裁出。返回与观察同一形状：`targets` 与 `metadata` 并列 |
| `declare_scope` / `get_scope` | 声明、查询本次任务的任务作用域 |
| `click` | 在某张截图的像素坐标处单击，须自报意图（`intent`）与危险性（`dangerous`）。注入前先把落点窗口带到前台 |
...
点击在命中测试与截图比对之后，还要过危险动作判定（ADR-0003、ADR-0004）：模型自报 `dangerous: true`，或服务端用 Windows 自带的 OCR 在落点附近读到高危词（删除、卸载、发送、确定、支付……），任一为真即判为危险。模型自报为危险的点击只在人于 Claude Code 里确认过之后执行，这一步由 PreToolUse hook 完成——**不注册 hook，自报为危险的动作一律执行不了**。模型自报不危险、但 OCR 读到高危词时，不要求把 `dangerous` 改成 true 再调一次，而是在这一次调用里弹出系统对话框问人；人确认后才执行，拒绝或超时则不执行。
...
hook 对自报为危险的调用答复 `ask`，并在 `%LOCALAPPDATA%\computer-use\tickets\` 留下只对这一次调用有效的裁决凭据；服务端凭它放行，一张只用一次，10 分钟后作废。OCR 命中而模型自报不危险时，hook 放行，服务端在同一次调用里用原生对话框问人。
