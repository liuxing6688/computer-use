...
class Pace:
    ...
    def wait_to_inject(self) -> None:
        """与上一次已经注入的输入动作隔开 `INPUT_INTERVAL`。第一次不用等。

        等待期间若急停，这一次不再注入。同时在等的其它输入也一起停，队列由此清空。
        """
...
def budget_verdict() -> Verdict:
    ...
    return Verdict((f"连续输入动作已达预算（{INPUT_BUDGET} 次）",))
